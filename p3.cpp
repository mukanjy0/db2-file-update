...
public:
    VariableRecordB(const char* filename) {
    ...
    void add(Enrollment record) {
        ...
        int sz {(int)record.code.size()};
        f.write(reinterpret_cast<char*>(&(sz)), sizeof(int));
        f.write(record.code.c_str(), sz);

        sz = sizeof(int);
        f.write(reinterpret_cast<char*>(&(sz)), sizeof(int));
        f.write(reinterpret_cast<char*>(&(record.semester)), sz);

        sz = sizeof(float);
        f.write(reinterpret_cast<char*>(&(sz)), sizeof(int));
        f.write(reinterpret_cast<char*>(&(record.monthlyPayment)), sz);

        sz = (int)record.observations.size();
        f.write(reinterpret_cast<char*>(&(sz)), sizeof(int));
        f.write(record.observations.c_str(), sz);
